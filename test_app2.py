...
def login_user(username, password):
    ...
    resp = make_request('GET', '/login/')
    if not (resp and resp.status_code == 200):
        print_failure("Failed to access login page")
        return False
    ...
    if resp and resp.status_code in (200, 302):
        if 'dashboard' in resp.url or 'Dashboard' in resp.text or resp.status_code == 302:
            ...
            if 'Invalid username or password' in resp.text:
                print_failure("Invalid username or password")
            else:
                print_failure("Login failed (unknown reason)")
            return False
    else:
...
def test_notes_crud():
    ...
    csrf_token = get_csrf_token(resp)
    if not csrf_token:
        return print_failure("Could not find CSRF token for note upload")
    ...
    if note_id_match:
        ...
        if resp and resp.status_code == 200:
            ...
            if csrf_token:
                ...
                print_failure("Could not find CSRF token for note deletion")
        else:
...
def test_tasks_crud():
    ...
    csrf_token = get_csrf_token(resp)
    if not csrf_token:
        return print_failure("Could not find CSRF token for task creation")
...
def test_attendance():
    ...
    csrf_token = get_csrf_token(resp)
    if not csrf_token:
        print_failure("Could not find CSRF token for attendance")
        return False
...
def test_planner():
    ...
    csrf_token = get_csrf_token(resp)
    if not csrf_token:
        print_failure("Could not find CSRF token for planner")
        return False
...
def main():
    ...
    # Login with the configured smoke-test account.
    username = os.environ.get('SMART_STUDY_TEST_USERNAME', 'testadmin')
    password = os.environ.get('SMART_STUDY_TEST_PASSWORD', 'admin123')
